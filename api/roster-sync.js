// Cron Job 1: Roster Sync — run every 24 hours
// Fetches all active MLB + NBA rosters and caches them
...
export default async function handler(req, res) {
  // Allow GET for cron pings, POST for manual triggers
  ...
  try {
    ...
    for (const team of MLB_TEAMS) {
      try {
        const r = await fetch(`https://statsapi.mlb.com/api/v1/teams/${team.id}/roster?rosterType=active&season=2025`);
        ...
        results.mlb[team.abbr] = { teamId: team.id, roster, updatedAt: new Date().toISOString() };
      } catch(e) {
        results.errors.push(`MLB ${team.abbr}: ${e.message}`);
      }
    }
    ...
    for (const team of NBA_TEAMS) {
      try {
        ...
        results.nba[team.abbr] = { teamId: team.id, roster, updatedAt: new Date().toISOString() };
      } catch(e) {
        results.errors.push(`NBA ${team.abbr}: ${e.message}`);
      }
    }

    // Store in Vercel KV or return for external storage
    console.log(`Roster sync complete: ${Object.keys(results.mlb).length} MLB teams, ${Object.keys(results.nba).length} NBA teams`);
