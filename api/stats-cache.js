export default async function handler(req, res) {
  ...
  const results = { mlb:{pitchers:{},batters:{},gamesCount:0}, nba:{players:{},gamesCount:0}, timestamp:new Date().toISOString(), errors:[] };
  ...
  try {
    ...
    for (const id of [...new Set(pitcherIds)]) {
      try {
        ...
        if (logs.length) results.mlb.pitchers[id] = {
      ...
      try {
        ...
        for (const hitter of hitters) {
          try {
            ...
            if (stat) results.mlb.batters[hitter.person?.id] = { name:hitter.person?.fullName, team:teamId, avg:stat.avg, hits:stat.hits, hr:stat.homeRuns, doubles:stat.doubles, tb:stat.totalBases, pa:stat.plateAppearances, k:stat.strikeOuts, cachedAt:new Date().toISOString() };
          } catch {}
        }
  ...
  try {
    ...
    const teamIds = [...new Set(events.flatMap(e=>e.competitions?.[0]?.competitors?.map(c=>c.team?.id)||[]).filter(Boolean))];
        ...
        for (const player of players) {
          try {
            ...
            if (gameKeys.length>0) {
              const games = gameKeys.map(key=>{ const ev=evts[key]; const stats=ev?.stats||[]; const gs={}; labels.forEach((l,i)=>{gs[l]=stats[i];}); return gs; }).filter(g=>Object.keys(g).length>0);
              const avg = f => { const vals=games.map(g=>parseFloat(g[f])||0); return vals.length?(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1):'0'; };
              results.nba.players[player.id] = { name:player.fullName, teamId, gamesPlayed:games.length, avgPTS:avg('PTS'), avgREB:avg('REB'), avgAST:avg('AST'), avg3PM:avg('3PM'), avgSTL:avg('STL'), avgBLK:avg('BLK'), avgMIN:avg('MIN'), cachedAt:new Date().toISOString() };
            }
          } catch {}
        }
