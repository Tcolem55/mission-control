...
function TopPicksSection({ games, gamesLoading, C }) {
  ...
  const generatePicks = async () => {
    ...
    try {
      ...
      // ── Step 0: Pre-fetch all rosters from schedule (most reliable source) ─
      log("Loading today's rosters from MLB schedule...");
      const scheduleRosters = {};
      try {
        const etDate = new Intl.DateTimeFormat("en-CA",{timeZone:"America/New_York"}).format(new Date());
        const sRes = await fetch(`https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=${etDate}&hydrate=roster(active,person),team`);
        const sData = await sRes.json();
        const PLIST = ["P","SP","RP","CL"];
        for (const game of sData.dates?.[0]?.games||[]) {
          for (const side of ["away","home"]) {
            const t = game.teams?.[side];
            const tid = t?.team?.id;
            const tn  = t?.team?.name;
            if (!tid || scheduleRosters[tid]) continue;
            const roster = t?.team?.roster?.roster || [];
            if (roster.length > 0) {
              scheduleRosters[tid] = roster
                .filter(p=>!PLIST.includes(p.position?.abbreviation))
                .map(p=>({id:p.person?.id, name:p.person?.fullName, pos:p.position?.abbreviation, team:tn}));
            }
          }
        }
        const cnt = Object.keys(scheduleRosters).length;
        log(cnt > 0 ? `✅ Rosters loaded for ${cnt} teams` : "⚠️ Schedule rosters empty");
      } catch(e) { log("⚠️ Schedule roster fetch failed"); }
      ...
      for (const game of games.slice(0, 6)) {
        ...
        // Use pre-fetched schedule rosters first (most reliable), fall back to direct API
        if (scheduleRosters[awayTeamId]?.length > 0) {
          awayHitters = scheduleRosters[awayTeamId];
          log(`✅ ${awayTeam}: ${awayHitters.length} hitters (schedule)`);
        } else {
          log(`Fetching roster for ${awayTeam}...`);
          try {
            const r = await fetch(`https://statsapi.mlb.com/api/v1/teams/${awayTeamId}/roster?rosterType=active&hydrate=person`);
            const d = await r.json();
            awayHitters = (d.roster||[]).filter(p=>!PITCHERS.includes(p.position?.abbreviation)).map(p=>({id:p.person?.id,name:p.person?.fullName,pos:p.position?.abbreviation,team:awayTeam}));
            log(`✅ ${awayTeam}: ${awayHitters.length} hitters (API)`);
          } catch(e) { log(`⚠️ ${awayTeam} roster failed`); }
        }
        if (scheduleRosters[homeTeamId]?.length > 0) {
          homeHitters = scheduleRosters[homeTeamId];
          log(`✅ ${homeTeam}: ${homeHitters.length} hitters (schedule)`);
        } else {
          log(`Fetching roster for ${homeTeam}...`);
          try {
            const r = await fetch(`https://statsapi.mlb.com/api/v1/teams/${homeTeamId}/roster?rosterType=active&hydrate=person`);
            const d = await r.json();
            homeHitters = (d.roster||[]).filter(p=>!PITCHERS.includes(p.position?.abbreviation)).map(p=>({id:p.person?.id,name:p.person?.fullName,pos:p.position?.abbreviation,team:homeTeam}));
            log(`✅ ${homeTeam}: ${homeHitters.length} hitters (API)`);
          } catch(e) { log(`⚠️ ${homeTeam} roster failed`); }
        }
...
Respond ONLY with valid JSON:
{
  ...
  const generateSlate = async () => {
    ...
    try {
      ...
      // Step 1: Statcast leaderboard once
      let statcastMap = {};
      try {
        const sc = await fetch("/api/statcast?type=leaderboard").then(r=>r.json());
        if (Array.isArray(sc)) sc.forEach(p => { if(p.id) statcastMap[String(p.id)] = p; });
      } catch {}

      // Step 2: Smart pre-filter — score each game, take best 7
      // Score based on park factor only (fast, no extra fetches)
      // This narrows 15 games → 7 best HR environments
      const scoredGames = games.map(game => {
        const venue = game.venue?.name || "";
        const pf = PARK_HR[venue] || 100;
        // Bonus for known pitcher names (TBD = less reliable)
        const hasPitchers = !!(game.teams?.away?.probablePitcher && game.teams?.home?.probablePitcher);
        return { game, pf, score: pf + (hasPitchers ? 5 : 0) };
      }).sort((a,b) => b.score - a.score);

      // Take top 7 by park factor — these are the best HR environments
      const selectedGames = scoredGames.slice(0,7).map(g => g.game);

      // Step 3: Process selected games in PARALLEL
      const gameResults = await Promise.allSettled(selectedGames.map(async game => {
        const away     = game.teams?.away;
        const home     = game.teams?.home;
        const awayId   = away?.team?.id;
        const homeId   = home?.team?.id;
        ...
        // Pitcher data + rosters all in parallel (4 fetches simultaneously)
        const getPitcher = async (pitcher) => {
          if (!pitcher?.id) return {hand:"R",name:"TBD",hr9:"—",hrVsL:0,hrVsR:0,era:"—"};
          try {
            ...
            return {hand, name:pitcher.fullName, hr9:((s.homeRuns||0)/ip*9).toFixed(2), hrVsL:vsL.homeRuns||0, hrVsR:vsR.homeRuns||0, era:s.era||"—"};
          } catch { return {hand:"R",name:pitcher.fullName||"TBD",hr9:"—",hrVsL:0,hrVsR:0,era:"—"}; }
        };
        ...
        const getRoster = async (teamId) => {
          try {
            ...
            return (d.roster||[]).filter(p=>!SKIP.includes(p.position?.abbreviation)).map(p=>({id:p.person?.id,name:p.person?.fullName,pos:p.position?.abbreviation}));
          } catch { return []; }
        };

        const [aPD, hPD, awayH, homeH] = await Promise.all([
          getPitcher(awaySP), getPitcher(homeSP),
          getRoster(awayId), getRoster(homeId),
        ]);

        // Batter stats — top 3 per team only, all parallel
        const getBatters = async (hitters, oppPitcher, spName, teamName) => {
          const oppHand = oppPitcher.hand;
          const res = await Promise.allSettled(hitters.slice(0,3).map(async h => {
            try {
              ...
              const iso    = s.slg&&s.avg?(parseFloat(s.slg)-parseFloat(s.avg)).toFixed(3):"—";
              const oppISO = opp.slg&&opp.avg?(parseFloat(opp.slg)-parseFloat(opp.avg)).toFixed(3):"—";
              const spHRvsHand = oppHand==="L"?oppPitcher.hrVsL:oppPitcher.hrVsR;
              const bp = sc.barrelPct, hh = sc.hardHitPct;
              const flags = [];
              if(bp>=12) flags.push("BARREL>12%");
              if(hh>=45) flags.push("HARDHIT>45%");
              if(parseFloat(oppPitcher.hr9)>=1.3) flags.push("SP HR/9>1.3");
              if(parkFactor>=108) flags.push(`PF ${parkFactor}`);
              return {
                name:h.name, pos:h.pos, team:teamName,
                game:`${awayName} @ ${homeName}`, venue, parkFactor, parkFlag,
                sp:spName, spHand:oppHand,
                oppHR9:oppPitcher.hr9, oppHRvsHand:spHRvsHand, oppPitcherERA:oppPitcher.era,
                seasonHR:s.homeRuns??0, iso,
                oppHR:opp.homeRuns??0, oppISO, oppAB:opp.atBats??0,
                last14HR:rec2.homeRuns??0, last14SLG:rec2.slg||"—",
                barrelPct:bp?`${bp}%`:"—", hardHitPct:hh?`${hh}%`:"—",
                xSLG:sc.xSLG||"—", avgEV:sc.avgEV?`${sc.avgEV}mph`:"—",
                sharpFlags:flags,
              };
            } catch { return null; }
          }));
        ...
        const [awayBatters, homeBatters] = await Promise.all([
          getBatters(awayH, hPD, homeSP?.fullName||"TBD", awayName),
          getBatters(homeH, aPD, awaySP?.fullName||"TBD", homeName),
        ]);

        return [...awayBatters, ...homeBatters];
      }));

      const allPlayers = gameResults.flatMap(r=>r.value||[]).filter(Boolean);

      const playerCtx = allPlayers.map(p =>
        `${p.name} (${p.team}, ${p.pos}) | Facing: ${p.sp} (${p.spHand}HP) | ${p.game} | Park:${p.venue} PF=${p.parkFactor} ${p.parkFlag} | SP HR/9=${p.oppHR9} ERA=${p.oppPitcherERA} allowed ${p.oppHRvsHand}HR vs ${p.spHand==="L"?"LHB":"RHB"} | Season:${p.seasonHR}HR ${p.iso}ISO | vs${p.spHand}HP:${p.oppHR}HR ${p.oppISO}ISO(${p.oppAB}AB) | L14:${p.last14HR}HR | Statcast:Barrel${p.barrelPct} HardHit${p.hardHitPct} xSLG${p.xSLG} EV${p.avgEV}`
      ).join("\n");

      const prompt = `MLB HR prop analyst. Rank TOP 6 HR candidates from today's best HR-environment games.

WEIGHTED FORMULA (apply strictly):
- 40% Batter Power: Barrel% + Hard Hit% (elite: >12% barrel, >45% HH)
- 30% Pitcher HR Risk: HR/9 >1.3 = vulnerable, hand-specific HR allowed
- 20% Park: PF>110 big boost, <90 = significant penalty
- 10% Recent form: L14 HR hot streak

SHARP = barrel>12% + hardhit>45% + SP HR/9>1.3 + PF>108 = near lock

CRITICAL VALIDATION:
- Each player's "Facing:" field shows their actual opposing pitcher
- A batter CANNOT face their own team's pitcher — verify before including
- Different teams for all 6 picks if possible, max 2 from same game

${allPlayers.length} PLAYERS FROM TOP HR-ENVIRONMENT GAMES TODAY:
${playerCtx}

Return JSON array only, no markdown:
[{"rank":1,"player":"Name","team":"Team","pos":"POS","game":"Away @ Home","venue":"Park","parkFactor":105,"parkFlag":"🔥 HITTER","sp":"Pitcher Name","spHand":"R","oppHR9":"1.52","oppHRvsHand":6,"seasonHR":10,"oppHR":4,"oppISO":"0.260","last14HR":3,"barrelPct":"13.8%","hardHitPct":"49.1%","xSLG":"0.550","avgEV":"92.1mph","sharpFlags":["BARREL>12%","HARDHIT>45%","SP HR/9>1.3","PF 112"],"hrOdds":"+190","score":91,"confidence":"HIGH","reason":"Specific stat-based reason referencing actual numbers"}]
Exactly 6 picks.`;
      ...
      const res = await fetch('/api/claude', {
        ...
        body: JSON.stringify({
          ...
          system:"Expert MLB HR analyst. Apply weighted formula. Validate each batter faces the opposing team pitcher only. Valid JSON array only.",
          messages:[{role:"user",content:prompt}]
        })
      });
      const data = await res.json();
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0b0d1a",overflow:"hidden",animation:"fadeUp 0.4s ease"}}>
      ...
      <div style={{flex:1,display:"flex",minHeight:0,overflow:"hidden"}}>
        ...
        {section==="HR PICKS" && sport==="MLB" && <HRTeamPicker C={MLB_C} games={mlbGames}/>}
...
// ── MOBILE APP ────────────────────────────────────────────────────────────────
// Full mobile layout with bottom tab bar, native app feel

function MobileApp({ extraProps, stocks, articles, weather, time, uptime, macroSnap, setShowMacros }) {
  const [activeTab, setActiveTab] = useState("HOME");
  const [sportSection, setSportSection] = useState("TODAY");
  const [sport, setSport] = useState("MLB");

  const C_HOME    = "#6366f1";
  const C_TRADING = "#00e5a0";
  const C_SPORTS  = "#f97316";
  const C_JOBS    = "#c084fc";
  const C_FINANCE = "#00b4ff";

  const TABS = [
    { id:"HOME",    icon:"⊞",  label:"Home",    color:C_HOME },
    { id:"TRADING", icon:"📡", label:"Trading", color:C_TRADING },
    { id:"SPORTS",  icon:"⚾",  label:"Sports",  color:C_SPORTS },
    { id:"JOBS",    icon:"◉",  label:"Jobs",    color:C_JOBS },
    { id:"MORE",    icon:"···", label:"More",   color:"#5a7090" },
  ];

  const tc = TABS.find(t=>t.id===activeTab)?.color || "#6366f1";

  return (
    <div style={{
      height:"100vh", width:"100vw", background:"#000408",
      display:"flex", flexDirection:"column", overflow:"hidden",
      fontFamily:"'Share Tech Mono',monospace", position:"relative",
    }}>
      <link href="https://fonts.googleapis.com/css2?family=Share+Tech+Mono&amp;family=Rajdhani:wght@600;700&amp;family=Bebas+Neue&amp;display=swap" rel="stylesheet"/>
      <style>{`
      ...
      <div style={{
        ...
        <div style={{fontSize:18,fontFamily:"'Bebas Neue',monospace",letterSpacing:5,color:"#00b4ff",textShadow:"0 0 15px rgba(0,180,255,0.5)"}}>
          MISSION<span style={{color:"rgba(0,255,180,0.8)"}}>·</span>CTRL
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          {weather && <div style={{fontSize:13}}>{weather.icon} <span style={{fontSize:11,color:"#00b4ff"}}>{weather.temp}°</span></div>}
          <div style={{fontSize:14,color:"#c8e8f8",fontFamily:"'Bebas Neue',monospace",letterSpacing:2}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        {activeTab==="HOME"    && <MobileHome extraProps={extraProps} stocks={stocks} articles={articles} weather={weather} macroSnap={macroSnap} setShowMacros={setShowMacros}/>}
        {activeTab==="TRADING" && <MobileTrading />}
        {activeTab==="SPORTS"  && <MobileSports />}
        {activeTab==="JOBS"    && <MobileJobs />}
        ...
        {TABS.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{
              ...
              <span style={{fontSize:9,letterSpacing:1,color:active?tab.color:"#3a5070",fontWeight:active?600:400}}>
                {tab.label}
              </span>
            </button>
          );
...
// ── MOBILE HOME ───────────────────────────────────────────────────────────────
function MobileHome({ extraProps, stocks, articles, weather, macroSnap, setShowMacros }) {
  ...
  const carbPct  = Math.min(Math.round((macroSnap.carbs/MACROS_GOAL.carbs)*100),100);
  const fatPct   = Math.min(Math.round((macroSnap.fat/MACROS_GOAL.fat)*100),100);

  if (activePanel) {
    return (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{flexShrink:0,padding:"10px 16px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12,background:"#020810"}}>
          <button onClick={()=>setActivePanel(null)} style={{background:"none",border:"none",color:"#00b4ff",fontSize:20,cursor:"pointer",padding:"4px 8px"}}>‹</button>
          <span style={{fontSize:14,color:"#c8d8f0",letterSpacing:2}}>{PANELS_CFG.find(p=>p.id===activePanel)?.label}</span>
        </div>
        <div style={{flex:1,overflow:"hidden"}}>
          <Panel
            cfg={PANELS_CFG.find(p=>p.id===activePanel)}
            isExpanded={true}
            onExpand={()=>{}}
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:10}}>
      ...
      {/* Nutrition bar - tap to log */}
      <button onClick={()=>setShowMacros(true)} style={{
        background:"linear-gradient(135deg,#0a0d1a,#060810)",
        border:"1px solid rgba(129,140,248,0.25)", borderRadius:12,
        padding:"14px 16px", cursor:"pointer", textAlign:"left", width:"100%",
      }}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{fontSize:11,color:"#818cf8",letterSpacing:2}}>⚡ NUTRITION</span>
          <span style={{fontSize:11,color:"#818cf8"}}>{kcalPct}% · Tap to log</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
          {[
            {label:"KCAL",   pct:kcalPct,  cur:macroSnap.kcal,    goal:MACROS_GOAL.kcal,    c:"#6366f1"},
            {label:"PROTEIN",pct:protPct,  cur:macroSnap.protein, goal:MACROS_GOAL.protein, c:"#f472b6"},
            {label:"CARBS",  pct:carbPct,  cur:macroSnap.carbs,   goal:MACROS_GOAL.carbs,   c:"#fbbf24"},
            {label:"FAT",    pct:fatPct,   cur:macroSnap.fat,     goal:MACROS_GOAL.fat,     c:"#00b4ff"},
          ].map(({label,pct,cur,goal,c})=>(
            <div key={label} style={{textAlign:"center"}}>
              <div style={{fontSize:8,color:"#3a5070",marginBottom:4,letterSpacing:1}}>{label}</div>
              <div style={{height:4,background:"rgba(255,255,255,0.06)",borderRadius:999,overflow:"hidden",marginBottom:4}}>
                <div style={{height:"100%",width:`${pct}%`,background:c,boxShadow:`0 0 6px ${c}`,borderRadius:999}}/>
              </div>
              <div style={{fontSize:10,color:c,fontWeight:600}}>{cur}<span style={{fontSize:8,color:"#3a5070"}}>/{goal}</span></div>
            </div>
          ))}
        </div>
      </button>

      {/* AI Panels — tap to open full screen */}
      {PANELS_CFG.map(p=>(
        <button key={p.id} onClick={()=>setActivePanel(p.id)} style={{
          background:"linear-gradient(135deg,#010408,#020810)",
          border:`1px solid ${p.color}25`, borderRadius:12,
          padding:"14px 16px", cursor:"pointer", textAlign:"left", width:"100%",
          display:"flex", alignItems:"center", gap:12,
        }}>
          <div style={{width:36,height:36,borderRadius:8,background:`${p.color}15`,border:`1px solid ${p.color}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
            {p.icon}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,color:p.color,letterSpacing:2,marginBottom:3}}>{p.label}</div>
            <div style={{fontSize:11,color:"#3a5070"}}>Tap to open →</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:p.color,boxShadow:`0 0 8px ${p.color}`,animation:"blink 2s infinite"}}/>
            <span style={{fontSize:8,color:`${p.color}50`,letterSpacing:2}}>LIVE</span>
          </div>
        </button>
      ))}
      ...
      {Object.keys(stocks).length>0 && (
        <div style={{background:"linear-gradient(135deg,#010408,#020810)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:12,padding:"14px 16px"}}>
          <div style={{fontSize:10,color:"#38bdf8",letterSpacing:3,marginBottom:10}}>📈 WATCHLIST</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {Object.entries(stocks).slice(0,6).map(([t,d])=>{
              const chg = ((d.price-(d.price/(1+d.changePct/100)))/( d.price/(1+d.changePct/100)))*100;
              const up = d.changePct>=0;
              return (
                <div key={t} style={{background:"#0d0f1e",borderRadius:8,padding:"8px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:11,color:"#c8d8f0",fontWeight:600,letterSpacing:1}}>{t}</span>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:11,color:"#c8d8f0"}}>${d.price?.toFixed(2)}</div>
                    <div style={{fontSize:10,color:up?"#818cf8":"#ff4444"}}>{up?"+":""}{d.changePct?.toFixed(2)}%</div>
                  </div>
                </div>
              );
            })}
      ...
      {/* News headlines */}
      {articles.length>0 && (
        <div style={{background:"linear-gradient(135deg,#010408,#020810)",border:"1px solid rgba(251,146,60,0.2)",borderRadius:12,padding:"14px 16px"}}>
          <div style={{fontSize:10,color:"#fb923c",letterSpacing:3,marginBottom:10}}>🌐 TOP NEWS</div>
          {articles.slice(0,4).map((a,i)=>(
            <div key={i} style={{padding:"8px 0",borderBottom:i<3?"1px solid rgba(255,255,255,0.05)":"none"}}>
              <div style={{fontSize:12,color:"#d4c8b8",lineHeight:1.5,marginBottom:3}}>{a.title}</div>
              <div style={{fontSize:10,color:"#fb923c"}}>{a.source?.name}</div>
            </div>
          ))}
        </div>
      )}
...
function MobileMore() {
  ...
  const items = [
    {id:"FINANCE", icon:"◈", label:"Finance",  color:"#00b4ff", desc:"Bills & subscriptions"},
    {id:"HEALTH",  icon:"♥", label:"Health",   color:"#f472b6", desc:"Coming soon"},
    {id:"TRAVEL",  icon:"◎", label:"Travel",   color:"#fbbf24", desc:"Coming soon"},
  ];

  if (activeTab==="FINANCE") return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{flexShrink:0,padding:"10px 16px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12,background:"#020810"}}>
        <button onClick={()=>setActiveTab(null)} style={{background:"none",border:"none",color:"#00b4ff",fontSize:20,cursor:"pointer",padding:"4px 8px"}}>‹</button>
        <span style={{fontSize:14,color:"#c8d8f0",letterSpacing:2}}>FINANCE</span>
      </div>
      <div style={{flex:1,overflow:"hidden"}}><FinanceTab /></div>
    </div>
  );

  return (
    <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:10}}>
      <div style={{fontSize:10,color:"#3a5070",letterSpacing:4,marginBottom:4}}>MORE MODULES</div>
      {items.map(item=>(
        <button key={item.id} onClick={()=>item.id==="FINANCE"&&setActiveTab(item.id)} style={{
          background:"linear-gradient(135deg,#010408,#020810)",
          border:`1px solid ${item.color}25`, borderRadius:12,
          padding:"16px", cursor:"pointer", textAlign:"left", width:"100%",
          display:"flex", alignItems:"center", gap:14,
        }}>
          <div style={{width:44,height:44,borderRadius:10,background:`${item.color}15`,border:`1px solid ${item.color}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>
            {item.icon}
          </div>
          <div>
            <div style={{fontSize:14,color:item.color,letterSpacing:2,marginBottom:3}}>{item.label}</div>
            <div style={{fontSize:11,color:"#3a5070"}}>{item.desc}</div>
          </div>
          <div style={{marginLeft:"auto",fontSize:18,color:"#3a5070"}}>›</div>
        </button>
      ))}
    </div>
  );
